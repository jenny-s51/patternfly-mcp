...
jest.mock('@patternfly/patternfly-component-schemas', () => ({
  componentNames: ['Button', 'Alert', 'Card', 'Modal'],
...
describe('componentSchemasTool', () => {
  ...
  describe('list action', () => {
    it('should list all available components', async () => {
      ...
      expect(response.totalComponents).toBe(4); // Mocked components
      expect(Array.isArray(response.components)).toBe(true);
      expect(response.components).toEqual(['Alert', 'Button', 'Card', 'Modal']); // Sorted mocked components
